...
[![Build Status](https://travis-ci.com/hotarugali/hexo-deployer-git-plus.svg?branch=master)](https://travis-ci.com/hotarugali/hexo-deployer-git-plus)
[![NPM version](https://badge.fury.io/js/hexo-deployer-git-plus.svg)](https://www.npmjs.com/package/hexo-deployer-git-plus)
[![Coverage Status](https://img.shields.io/coveralls/hotarugali/hexo-deployer-git-plus.svg)](https://coveralls.io/r/hotarugali/hexo-deployer-git-plus?branch=master)


This git deployer plus plugin is a fork of [hexo-deployer-git](https://github.com/hexojs/hexo-deployer-git). Add one option for deploying of git.
- **privateKey**: Path of private key generated by `ssh-keygen`. User can just use the default global private key without settting **privateKey** or specify other private key by setting **privateKey**.

Other options are the same with the [hexo-deployer-git](https://github.com/hexojs/hexo-deployer-git).
...
``` bash
$ npm install hexo-deployer-git-plus --save
```
...
`hexo-deployer-git-plus` works by generating the site in `.deploy_git` and *force pushing* to the repo(es) in config.
